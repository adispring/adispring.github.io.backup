---
title: 函数式编程中的函数们
date: 2017-10-26 22:10:08
tags:
---

函数式编程中函数有三种不同的解读方式，包括纯函数、高阶函数和一等函数。下面分别对这三者的概念、联系和应用进行详解。
...
因此，执行一段应用透明的代码（函数），对于相同的参数，总是给出相同的结果（如果结果不同，就会改变整个程序的行为）。我们也称这样的函数（代码）为纯函数。

应用透明的一个典型应用即函数缓存。我们可以将已经执行过的输入值缓存，下次调用时，若输入值相同，直接用缓存结果代替计算过程返回即可。
...
作为参数或返回值的函数，也即上一节的一等函数。高阶函数以一等函数作为基础，只有支持一等函数的语言才能进行高阶函数编程。
...
我们抽象除了最一般的 `ajax` 请求函数，在具体应用时，我们用能通过函数工厂生产出作用不同的函数。
